...
const MenuItem = ({ id, name, price, description, image, isVeg, votes, restaurant }) => {
    const { addToCart, cartItems, updateQuantity } = useCart();

    const cartItem = cartItems.find(item => item.id === id);

    const handleAdd = () => {
        addToCart({ id, name, price, image, isVeg }, restaurant);
    };
                ...
                <div className="absolute -bottom-3 left-1/2 -translate-x-1/2 w-24">
                    {cartItem ? (
                        <div className="flex items-center justify-between bg-white text-green-600 border border-gray-200 shadow-lg rounded-xl h-9 px-2 font-bold text-sm">
                            <button onClick={() => updateQuantity(id, 'minus')} className="px-2 text-gray-400 hover:text-green-600">-</button>
                            <span>{cartItem.quantity}</span>
                            <button onClick={() => updateQuantity(id, 'plus')} className="px-2 hover:scale-110">+</button>
                        </div>
                    ) : (
                        <Button
                            variant="primary"
